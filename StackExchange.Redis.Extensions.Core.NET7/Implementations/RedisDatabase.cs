...
using StackExchange.Redis.Extensions.Core.NET7.Models;
using StackExchange.Redis.Extensions.Core.NET7.ServerIteration;
...
public partial class RedisDatabase : IRedisDatabase
{
        ...
        string? keyPrefix = null)
    {
        ...
        this.connectionPoolManager =
            connectionPoolManager ?? throw new ArgumentNullException(nameof(connectionPoolManager));
    ...
    /// <inheritdoc />
    public Task<bool> ReplaceAsync<T>(string key, T value, When when = When.Always,
    ...
    /// <inheritdoc />
    public Task<bool> AddAsync<T>(string key, T value, DateTimeOffset expiresAt, When when = When.Always,
        CommandFlags flag = CommandFlags.None, HashSet<string>? tags = null)
    {
        var entryBytes = value.OfValueSize(Serializer, maxValueLength, key);
        ...
        return tags?.Count > 0
            ? ExecuteAddWithTagsAsync(key, tags, db => db.StringSetAsync(key, entryBytes, expiration, when, flag), when,
                flag)
            : Database.StringSetAsync(key, entryBytes, expiration, when, flag);
    }

    /// <inheritdoc />
    public Task<bool> ReplaceAsync<T>(string key, T value, DateTimeOffset expiresAt, When when = When.Always,
        CommandFlags flag = CommandFlags.None)
    {
    ...
    /// <inheritdoc />
    public Task<bool> AddAsync<T>(string key, T value, TimeSpan expiresIn, When when = When.Always,
        CommandFlags flag = CommandFlags.None, HashSet<string>? tags = null)
    {
        var entryBytes = value.OfValueSize(Serializer, maxValueLength, key);

        return tags?.Count > 0
            ? ExecuteAddWithTagsAsync(key, tags, db => db.StringSetAsync(key, entryBytes, expiresIn, when, flag), when,
                flag)
            : Database.StringSetAsync(key, entryBytes, expiresIn, when, flag);
    }

    /// <inheritdoc />
    public Task<bool> ReplaceAsync<T>(string key, T value, TimeSpan expiresIn, When when = When.Always,
        CommandFlags flag = CommandFlags.None)
    {
    ...
    /// <inheritdoc />
    public Task<bool> AddAllAsync<T>(Tuple<string, T>[] items, When when = When.Always,
        CommandFlags flag = CommandFlags.None)
    {
        var values = items
    ...
    /// <inheritdoc />
    public async Task<bool> AddAllAsync<T>(Tuple<string, T>[] items, DateTimeOffset expiresAt, When when = When.Always,
        CommandFlags flag = CommandFlags.None)
    {
        var values = items
            .OfValueInListSize(Serializer, maxValueLength)
    ...
    /// <inheritdoc />
    public async Task<bool> AddAllAsync<T>(Tuple<string, T>[] items, TimeSpan expiresOn, When when = When.Always,
        CommandFlags flag = CommandFlags.None)
    {
        var values = items
            .OfValueInListSize(Serializer, maxValueLength)
    ...
    public async Task<IEnumerable<string>> SearchKeysAsync(string pattern)
    {
        ...
        foreach (var unused in servers)
        {
            ...
            do
            {
                var redisResult = await Database
                    .ExecuteAsync("SCAN", nextCursor.ToString(), "MATCH", pattern, "COUNT", "1000")
                    .ConfigureAwait(false);
    ...
    /// <inheritdoc />
    public Task<double> SortedSetAddIncrementAsync<T>(string key, T? value, double score,
    ...
    private InfoDetail[] ParseCategorizedInfo(string info)
    {
        ...
        foreach (var line in lines.Where(x => !string.IsNullOrWhiteSpace(x)))
        {
            ...
            if (idx > 0)
            {
                ...
                data.Add(new InfoDetail(category, key, infoValue));
            }
        }
