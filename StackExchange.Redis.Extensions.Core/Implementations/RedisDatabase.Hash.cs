...
public partial class RedisDatabase
{
    ...
    /// <inheritdoc />
    public async Task<Dictionary<string, T?>> HashGetAsync<T>(string hashKey, string[] keys,
    ...
    public Task<bool> HashSetAsync<T>(string hashKey, string key, T value, bool nx = false,
        CommandFlags commandFlags = CommandFlags.None)
    {
        return Database.HashSetAsync(hashKey, key, Serializer.Serialize(value), nx ? When.NotExists : When.Always,
            commandFlags);
    }
    ...
    {
        return (await Database.HashValuesAsync(hashKey, commandFlags).ConfigureAwait(false)).Select(x =>
    ...
    public Dictionary<string, T> HashScan<T>(string hashKey, string pattern, int pageSize = 10,
        CommandFlags commandFlags = CommandFlags.None)
    {
        return Database.HashScan(hashKey, pattern, pageSize, commandFlags).ToDictionary(x => x.Name.ToString(),
            x => Serializer.Deserialize<T>(x.Value), StringComparer.Ordinal);
    }
}